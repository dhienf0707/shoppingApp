...
# import ssl for disabling ssl certificate verification
import ssl
...
### download the archive html file (motherboard)
##archive_site = 'https://www.umart.com.au/Graphics-Cards---GPU_610C.html'
...
##gpu_html = open('archive\gpu.html', 'w', encoding = 'UTF-8')
##gpu_html.write(archive_contents)
##gpu_html.close()

# Archived data
# gpu
# open the archived html file
gpu_html = open('archive\gpu.html', 'U').read()

# find product's name
gpu_name = findall('alt="(.*?)"', gpu_html)

# find product's price
gpu_price = findall('itemprop="price" content="(.*?)"', gpu_html)
...
gpu_image = findall('title=".*?" ><img src="(.*?)" alt', gpu_html)
...
for index in range(len(phone_name)):
    temp += [sub('\(','\(', phone_name[index])]
    temp[index] = sub('\)','\)', temp[index])
    temp[index] = sub('\|','\|', temp[index])
    temp[index] = sub('\+','\+', temp[index])
    
phone_image = []
for each in temp:
    phone_image += findall('title="' + each + '".*?<img src="(.*?) alt="' + each,phone_contents)
...
# washing machines
# download washing machines contents from url
washing_machine_contents = download('https://www.betta.com.au/laundry/washing-machines', 'washing_machine')

# find product's name
washing_machine_name = findall('<img class="first_image" src=".*?" alt="(.*?)">',
                               washing_machine_contents)
...
# functions for main window and adding value to product table
...
def take_focus():
    ...
    keyboard_event(ctr_key, 0, extended_key | 0, 0) # press the button
    # make window a foreground thread which give the input of keyboard
    # into this window
...
# GPU radio button
gpu = Radiobutton(archive_frame, text = 'Graphics cards',
...
washing_machine = Radiobutton(online_frame, text = 'Washing machines',
                  variable = category, value = 3,
                  command = insert_value)
washing_machine.grid(row = 2, column =  1, sticky = 'w')
...
# functions for interaction with cart list window and its widgets
...
def cart_select_item(b):
    ...
    # and the selected product has to be in the cart list
    if selected_item['values'] != '':
        delete['state'] = NORMAL
        # position of possible deleted item
